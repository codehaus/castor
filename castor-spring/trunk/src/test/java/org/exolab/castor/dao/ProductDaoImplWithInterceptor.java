...
public class ProductDaoImplWithInterceptor extends CastorTemplate implements
		ProductDaoWithInterceptor {
	...
	public Product loadProduct(final int id) throws ObjectNotFoundException {
		...
		try {
			database.begin();
			Product product = (Product) database.load(Product.class,
					new Integer(1));
			database.commit();
			return product;
